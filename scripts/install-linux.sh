...
# Runs a command with a wall-clock limit. `timeout` is coreutils and is not on
# every system, so fall back to a background watchdog. Returns 124 on timeout,
# matching what `timeout` itself reports.
run_with_timeout() {
